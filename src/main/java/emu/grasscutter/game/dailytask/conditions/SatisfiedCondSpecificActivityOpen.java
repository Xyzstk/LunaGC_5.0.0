...
@DailyTaskValueCond(ConditionType.CONDITION_SPECIFIC_ACTIVITY_OPEN)
public class SatisfiedCondSpecificActivityOpen extends BaseSatisfiedCond {
    public boolean execute(Player owner, DailyTaskData.DailyTaskSatisfiedCond condition) {
        int activityId = condition.getParam()[0];
        return ActivityManager.isActivityOpen(activityId);
    }
}
